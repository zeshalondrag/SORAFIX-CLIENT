...
export default function RequestDetailPage() {
  ...
  const handleConfirmPrice = async (isConfirmed: boolean) => {
    setConfirmingPrice(true);
    try {
      await requestsApi.confirmRequestPrice(requestId, isConfirmed);
      if (isConfirmed) {
        const { paymentUrl } = await requestsApi.generatePayment(requestId);
        if (paymentUrl) {
          await openAttachmentUrl(paymentUrl, `payment-request-${requestId}`);
        }
      }
      setPriceConfirmModalVisible(false);
      showSuccess(
        isConfirmed
          ? 'Стоимость подтверждена. Ссылка на оплату отправлена в чат и Telegram.'
          : 'Стоимость отклонена, ожидайте новое предложение'
      );
      refreshNotifications();
      loadData();
    } catch {
      showError('Ошибка подтверждения стоимости');
    } finally {
  ...
  // Определяем менеджера для заголовка чата
  const managerMessage = messages.find((m) => m.userId !== currentUserId && m.roleName === 'Менеджер');
  const managerName = managerMessage
    ? getUserDisplayFromName(managerMessage.firstName, managerMessage.lastName)
    : 'Менеджер';
  const managerInitials = managerMessage
    ? getUserInitialsFromName(managerMessage.firstName, managerMessage.lastName)
    : 'М';

  // Определяем специалиста для заголовка чата
  ...
  return (
    ...
    >
      ...
      <View style={[styles.mainContent, isPhone && styles.mainContentPhone]}>
        {/* Чат — основной контент; на телефоне на всю ширину */}
        <View style={[styles.chatSection, isPhone && styles.chatSectionPhone]}>
          {/* Заголовок чата - Менеджер + Специалист */}
          <View style={[styles.chatHeader, isPhone && styles.chatHeaderPhone]}>
            <UserAvatar initials={managerInitials} size={isPhone ? 30 : 36} />
            <View style={styles.chatHeaderInfo}>
              <Text style={[styles.chatHeaderName, isPhone && styles.chatHeaderNamePhone]}>{managerName}</Text>
              <Text style={[styles.chatHeaderRole, isPhone && styles.chatHeaderRolePhone]}>Менеджер</Text>
            </View>
            {specialistName && (
              <>
            ...
            <View style={[styles.inputContainer, isPhone && styles.inputContainerPhone]}>
              {pendingAttachment && (
                <View style={[styles.pendingAttachmentBar, isPhone && styles.pendingAttachmentBarPhone]}>
                  ...
                  <Text numberOfLines={1} style={[styles.pendingAttachmentName, isPhone && styles.pendingAttachmentNamePhone]}>
        ...
        {!isPhone && (
          <View style={styles.infoSection}>
            <ScrollView showsVerticalScrollIndicator={false}>
              <ClientRequestDetailsScrollBody
                request={request}
                ...
                onCancelRequest={() => setCancelModalVisible(true)}
                onConfirmPricePress={() => setPriceConfirmModalVisible(true)}
              />
            </ScrollView>
          </View>
        )}
        ...
        <View style={styles.actionMenuOverlay} pointerEvents="box-none">
          ...
          <View
            style={[
              styles.actionMenu,
              {
                left: Math.max(12, actionMenuPosition.x - 415),
                top: Math.max(12, actionMenuPosition.y - 200),
              },
            ]}
          >
            <TouchableOpacity
      ...
      <Modal visible={priceConfirmModalVisible} transparent animationType="fade">
        <View style={modalStyles.overlay}>
          <View style={modalStyles.box}>
            <Text style={modalStyles.title}>Подтверждение стоимости</Text>
            <Text style={styles.confirmText}>
              Вы согласны с назначенной стоимостью {formatPrice(getPrice(request))}?
            </Text>
            <View style={modalStyles.buttons}>
              <TouchableOpacity
                style={[styles.priceRejectButton, confirmingPrice && modalStyles.buttonDisabled]}
                onPress={() => handleConfirmPrice(false)}
                disabled={confirmingPrice}
              >
                <Text style={styles.priceRejectButtonText}>Нет, не согласен</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[modalStyles.button, confirmingPrice && modalStyles.buttonDisabled]}
                onPress={() => handleConfirmPrice(true)}
                disabled={confirmingPrice}
              >
                <Text style={modalStyles.buttonText}>{confirmingPrice ? 'Сохранение...' : 'Согласен'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
type ClientRequestDetailsScrollBodyProps = {
  request: Request;
  ...
  onConfirmPricePress: () => void;
};

function ClientRequestDetailsScrollBody({
  request,
  employeeInfo,
  statusHistory,
  ...
  onCancelRequest,
  onConfirmPricePress,
}: ClientRequestDetailsScrollBodyProps) {
  const statusId = getStatusId(request);
      ...
      <View style={styles.infoCard}>
        ...
        <View style={styles.infoFieldRow}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            {isPriceConfirmed && <Text style={styles.infoFieldValue}>{price}</Text>}
            {isWaitingPriceConfirmation && (
              <TouchableOpacity style={styles.priceConfirmButton} onPress={onConfirmPricePress}>
                <Text style={styles.priceConfirmButtonText}>
                  Подтвердить — {formatPrice(getPrice(request))}
                </Text>
              </TouchableOpacity>
            )}
        ...
        <View style={styles.infoFieldRow}>
          ...
          {employeeInfo ? (
            <View style={styles.userInlineCard}>
              <UserAvatar
                initials={getUserInitialsFromName(employeeInfo.firstName, employeeInfo.lastName)}
                size={32}
              />
              <Text style={styles.userInlineName}>
                {employeeInfo.firstName} {employeeInfo.lastName}
              </Text>
            </View>
          ) : (
            <Text style={styles.infoFieldValueMuted}>Уточняется</Text>
          )}
        </View>
          ...
          <View style={styles.photosRow}>
            {requestGalleryPhotos.map((photo, idx) => (
              <TouchableOpacity
                ...
                activeOpacity={0.8}
              >
                <Image source={{ uri: getPhotoUrl(photo) }} style={styles.photoImage} resizeMode="cover" />
              </TouchableOpacity>
            ))}
          </View>
        )}
          ...
          statusHistory.map((item, index) => {
            ...
            return (
              <View key={item.id} style={styles.historyItem}>
                <View
                  style={[styles.historyDot, { backgroundColor: isActive ? itemStatusColor : '#D1D5DB' }]}
                />
                <View style={styles.historyContent}>
                  <Text style={[styles.historyStatus, isActive && { color: itemStatusColor }]}>
                    {itemStatusLabel}
                  </Text>
                  <View style={styles.historyMeta}>
                    {itemStatusDesc && (
