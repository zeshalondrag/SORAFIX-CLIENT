...
function getSpecialistNotifDisplay(type: string): {
  ...
  switch (type) {
    case 'specialist_assigned':
      return { iconName: 'clipboard-account-outline', iconColor: '#3B82F6', title: 'Назначенная заявка' };
    case 'status_changed':
...
function NotificationItem({ notification, onPress, isPhone }: NotificationItemProps) {
  ...
  const { iconName, iconColor, title } = getSpecialistNotifDisplay(type);
  ...
  return (
    <View style={[styles.emptyContainer, compact && styles.emptyContainerPhone]}>
      ...
      <Text style={styles.emptyHint}>
        Здесь будут отображаться уведомления о назначенных заявках
      </Text>
    </View>
  );
}
...
export default function NotificationsPage() {
  ...
  const handleGoToRequest = () => {
    ...
    handleCloseModal();
    if (reqId > 0) {
      router.push(`/(specialist)/request/${reqId}` as '/');
    }
  };
