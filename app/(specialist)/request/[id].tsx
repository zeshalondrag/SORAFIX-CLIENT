...
export default function SpecialistRequestDetailPage() {
  ...
  const emailVerified = user?.emailVerified ?? (user as unknown as { email_verified?: boolean })?.email_verified ?? false;
  ...
  const handleCompleteRequest = async () => {
    ...
    try {
      ...
      showSuccess('Выполнение заявки завершено');
      setCompleteModalVisible(false);
      refreshNotifications();
      loadData();
    } catch {
      showError('Ошибка завершения выполнения');
    } finally {
      setCompleting(false);
    }
  };
  ...
  const price = formatPrice(getPrice(request));
  const canSendMessage = (!!messageText.trim() || pendingAttachment != null) && !sending;

  const managerMsg = messages.find((m) => m.roleName === 'Менеджер');
  const managerName = managerMsg
    ? getUserDisplayFromName(managerMsg.firstName, managerMsg.lastName)
    : 'Менеджер';
  const managerInitials = managerMsg
    ? getUserInitialsFromName(managerMsg.firstName, managerMsg.lastName)
    : 'М';
  ...
  return (
    ...
    >
        ...
        {!isPhone && (
          <View style={styles.infoSection}>
            <ScrollView showsVerticalScrollIndicator={false}>
              <SpecialistRequestSidebarBody {...specialistSidebarProps} />
            </ScrollView>
          </View>
        )}
      </View>
      ...
      <Modal visible={completeModalVisible} transparent animationType="fade">
        <View style={modalStyles.overlay}>
          <View style={modalStyles.box}>
            <Text style={modalStyles.title}>Завершить выполнение</Text>
            <Text style={styles.confirmText}>
              Вы уверены, что хотите завершить выполнение заявки «{request.title}»?
              Статус заявки будет изменён на «Проверка».
            </Text>
            <View style={modalStyles.buttons}>
              <TouchableOpacity style={modalStyles.buttonSecondary} onPress={() => setCompleteModalVisible(false)}>
                <Text style={modalStyles.buttonSecondaryText}>Отмена</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[modalStyles.button, completing && modalStyles.buttonDisabled]}
                onPress={handleCompleteRequest}
                disabled={completing}
              >
                <Text style={modalStyles.buttonText}>{completing ? 'Завершение...' : 'Завершить'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      {/* Модальное окно: почта не подтверждена */}
      <Modal visible={emailNotVerifiedModal} transparent animationType="fade">
        <View style={modalStyles.overlay}>
          <View style={modalStyles.box}>
            ...
            <Text style={styles.confirmText}>
              Для отправки сообщений необходимо подтвердить электронную почту.
              Перейдите в профиль для подтверждения.
            </Text>
            <View style={modalStyles.buttons}>
              <TouchableOpacity style={modalStyles.buttonSecondary} onPress={() => setEmailNotVerifiedModal(false)}>
                <Text style={modalStyles.buttonSecondaryText}>Закрыть</Text>
              </TouchableOpacity>
              <TouchableOpacity style={modalStyles.button} onPress={() => { setEmailNotVerifiedModal(false); router.push('/(specialist)/profile' as '/'); }}>
...
type SpecialistRequestSidebarProps = {
  request: Request;
...
function SpecialistRequestSidebarBody({
  request,
  clientInfo,
  employeeInfo,
  statusHistory,
  ...
  return (
    <>
      <View style={styles.infoCard}>
        ...
        {statusId === 3 && (
          <TouchableOpacity style={styles.completeButton} onPress={onCompletePress}>
            <MaterialIcons name="check-circle" size={18} color="#FFFFFF" />
            <Text style={styles.completeButtonText}>Завершить выполнение</Text>
          </TouchableOpacity>
        )}
        {statusId === 4 && (
          <View style={styles.waitingVerifyButton}>
            <MaterialIcons name="hourglass-top" size={18} color={Colors.light.link} />
            <Text style={styles.waitingVerifyButtonText}>Ожидание проверки менеджером</Text>
          </View>
        )}
      </View>
