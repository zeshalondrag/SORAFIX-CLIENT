...
const NAV_ITEMS: NavItem[] = [
  { href: '/(specialist)', icon: 'view-dashboard-outline', label: 'Личный кабинет' },
  { href: '/(specialist)/requests', icon: 'clipboard-text-outline', label: 'Назначенные заявки' },
  { href: '/(specialist)/profile', icon: 'account-outline', label: 'Профиль' },
  { href: '/(specialist)/notifications', icon: 'bell-outline', label: 'Уведомления' },
];
...
export default function SpecialistLayout() {
  ...
  const isSpecialistRequestDetail =
  ...
  useEffect(() => {
    ...
    if (roleId === SPECIALIST_ROLE_ID) return;
  ...
  const roleName = user.role?.name ?? 'Специалист';

  const getCurrentTab = () => {
    const p = (pathname ?? '').toLowerCase();
    if (p.includes('requests')) return 'Назначенные заявки';
  ...
  const TAB_DESCRIPTIONS: Record<string, string> = {
    'Личный кабинет': 'Обзор назначенных заявок',
    'Назначенные заявки': 'Список назначенных вам заявок',
            ...
            <View style={styles.navItems}>
              {NAV_ITEMS.map((item) => (
                <NavLink
                  ...
                  badge={item.label === 'Уведомления' ? unreadCount : undefined}
                />
              ))}
            </View>
            <View style={styles.sidebarDivider} />
      ...
      <View style={styles.main}>
        ...
        <View
          style={[
            ...
            isSpecialistRequestDetail && styles.contentFullBleedChat,
          ]}
        >
