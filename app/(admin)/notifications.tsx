...
type AdminNotifType = 'new_client' | 'role_changed' | 'other';

function getAdminNotifType(message: string): AdminNotifType {
  if (message.includes('Новый клиент')) return 'new_client';
  if (message.includes('назначена роль')) return 'role_changed';
  return 'other';
}

function getAdminNotifDisplay(type: AdminNotifType): {
  ...
  switch (type) {
    case 'new_client':
      return { iconName: 'account-plus-outline', iconColor: Colors.light.button, title: 'Новый клиент' };
    case 'role_changed':
      return { iconName: 'account-convert-outline', iconColor: '#F59E0B', title: 'Роль изменена' };
    default:
      return { iconName: 'bell-outline', iconColor: Colors.light.link, title: 'Уведомление' };
  }
}
...
function NotificationItem({ notification, onPress, isPhone }: NotificationItemProps) {
  ...
  const type = getAdminNotifType(notification.message);
  const { iconName, iconColor, title } = getAdminNotifDisplay(type);
        ...
        <View style={styles.notifInfo}>
          ...
          {isPhone ? (
            ...
            <View style={styles.notifMeta}>
              <Text style={styles.notifMetaText} numberOfLines={1}>{notification.message}</Text>
  ...
  return (
    <View style={[styles.emptyContainer, compact && styles.emptyContainerPhone]}>
      ...
      <Text style={styles.emptyHint}>
        Здесь будут отображаться уведомления о новых клиентах и изменениях ролей
      </Text>
    </View>
  );
}
...
function NotificationModal({ visible, notification, onClose, isPhone }: NotificationModalProps) {
    ...
    <Modal visible={visible} transparent animationType="fade">
      <View style={[modalStyles.overlay, isPhone && modalStyles.overlayPhone]}>
        <View style={[modalStyles.box, isPhone && modalStyles.boxPhone]}>
          ...
          >
            <View style={[modalStyles.iconRow, isPhone && modalStyles.iconRowPhone]}>
              <View style={[styles.notifIconWrap, { backgroundColor: iconColor + '20' }]}>
                <MaterialCommunityIcons name={iconName} size={isPhone ? 24 : 28} color={iconColor} />
              </View>
              <Text style={[modalStyles.typeTitle, isPhone && modalStyles.typeTitlePhone]}>{title}</Text>
            </View>
