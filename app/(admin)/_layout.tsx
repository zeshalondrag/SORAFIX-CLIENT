...
const NAV_ITEMS: NavItem[] = [
  { href: '/(admin)', icon: 'chart-bar', label: 'Аналитика' },
  { href: '/(admin)/audit', icon: 'shield-search', label: 'Аудит' },
  { href: '/(admin)/users', icon: 'account-group-outline', label: 'Пользователи' },
  { href: '/(admin)/profile', icon: 'account-outline', label: 'Профиль' },
  { href: '/(admin)/notifications', icon: 'bell-outline', label: 'Уведомления' },
];
...
export default function AdminLayout() {
  ...
  useEffect(() => {
    ...
    if (roleId === CLIENT_ROLE_ID) router.replace('/(client)' as '/');
  ...
  const roleName = user.role?.name ?? 'Администратор';

  const getCurrentTab = () => {
    const p = (pathname ?? '').toLowerCase();
    if (p.includes('audit')) return 'Аудит';
  ...
  return (
    <View style={styles.root}>
      ...
      <Animated.View
        style={[
          styles.sidebar,
          { paddingTop: isNarrow ? insets.top + 8 : 12 },
          {
            width: isNarrow
        ...
        <View style={styles.header}>
          {isNarrow && (
            <TouchableOpacity
              ...
              style={[styles.menuButton, { marginTop: insets.top }]}
            >
