...
const NAV_ITEMS: NavItem[] = [
  { href: '/(manager)', icon: 'view-dashboard-outline', label: 'Личный кабинет' },
  { href: '/(manager)/requests', icon: 'clipboard-text-outline', label: 'Заявки клиентов' },
  { href: '/(manager)/profile', icon: 'account-outline', label: 'Профиль' },
  { href: '/(manager)/notifications', icon: 'bell-outline', label: 'Уведомления' },
];
...
export default function ManagerLayout() {
  ...
  useEffect(() => {
    ...
    if (roleId === MANAGER_ROLE_ID) return;
  ...
  const roleName = user.role?.name ?? 'Менеджер';

  const getCurrentTab = () => {
    const p = (pathname ?? '').toLowerCase();
    if (p.includes('requests')) return 'Заявки клиентов';
  ...
  const TAB_DESCRIPTIONS: Record<string, string> = {
    'Личный кабинет': 'Обзор и управление',
    'Заявки клиентов': 'Все заявки клиентов',
      ...
      <View style={styles.main}>
        ...
        <View
          style={[
            ...
            isManagerRequestDetail && styles.contentFullBleedChat,
          ]}
        >
