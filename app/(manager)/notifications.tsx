...
function getManagerNotifDisplay(type: string): {
  ...
  switch (type) {
    case 'client_request_created':
      return { iconName: 'file-document-check-outline', iconColor: Colors.light.button, title: 'Заявка клиента' };
    ...
    case 'specialist_accepted':
      return { iconName: 'clipboard-check-outline', iconColor: Colors.light.button, title: 'Заявка принята специалистом' };
    case 'specialist_completed':
      return { iconName: 'check-circle-outline', iconColor: Colors.light.button, title: 'Заявка выполнена' };
    default:
      return { iconName: 'file-document-check-outline', iconColor: Colors.light.button, title: 'Заявка клиента' };
  }
}
...
function NotificationItem({ notification, onPress, isPhone }: NotificationItemProps) {
  ...
  const { iconName, iconColor, title } = getManagerNotifDisplay(type);
...
export default function NotificationsPage() {
  ...
  const handleGoToRequest = () => {
    ...
    handleCloseModal();
    if (reqId > 0) {
      router.push(`/(manager)/request/${reqId}` as '/');
    }
  };
